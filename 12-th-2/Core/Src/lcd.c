...
uint16_t nowPage=DATA_PAGE,lastPage=-1;
uint16_t freq,rValue=2;
double voltage,duty;

void LCD_Show(){
	char tem[21]={0};
	uint16_t adcValue=HAL_ADC_GetValue(&hadc2);
	duty=adcValue/4095.0;
	...
	if(nowPage==DATA_PAGE){
		...
		sprintf(tem,"   FRQ:%dHz     ",freq);
		LCD_DisplayStringLine(Line4,(uint8_t *)tem);
		sprintf(tem,"   R37:%.2fV    ",voltage);
		LCD_DisplayStringLine(Line6,(uint8_t *)tem);
	}
	else{
		LCD_DisplayStringLine(Line2,(uint8_t *)"       Para");
		sprintf(tem,"      R:%d    ",rValue);
		LCD_DisplayStringLine(Line4,(uint8_t *)tem);
	}
}
